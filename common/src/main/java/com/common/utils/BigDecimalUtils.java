...
public class BigDecimalUtils {
    ...
    public static String add(String v1, String v2, int scale, int round) {
        if (scale < 0) {
        ...
        BigDecimal b1 = new BigDecimal(v1);
        BigDecimal b2 = new BigDecimal(v2);
        return b1.add(b2).setScale(scale, round).toString();
    }
    ...
    public static String multiply(String v1, String v2) {
        return multiply(v1, v2, DEF_DIV_SCALE, BigDecimal.ROUND_HALF_UP);
    }
     ...
     * @return 两个参数的积
     */
    public static String multiply(String v1, String v2, int scale, int roundingMode) {
        if (scale < 0) {
            throw new IllegalArgumentException(
                    "The scale must be a positive integer or zero");
        }
    ...
    public static String round(String v, int scale, int roundingMode) {
        ...
        String result = b.setScale(scale, roundingMode).toString();
        if ("0E-8".equals(result)) result = "0";
        return result;
    }
     ...
     *
     * @author lzy
     * create at 2018/10/11 21:03
     **/
    public static String stripTrailingZerosAndPlanString(float src) {
        return new BigDecimal(src).stripTrailingZeros().toPlainString();
    }

    public static String stripTrailingZerosAndPlanString(float src, int scale) {
        return new BigDecimal(src).setScale(scale, BigDecimal.ROUND_HALF_UP).stripTrailingZeros().toPlainString();
    }


    public static float toFloat(String src) {
        if (StringUtils.isEmpty(src)) return 0F;
        if (src.startsWith(".")) src = "0" + src;
        return new BigDecimal(src).floatValue();
    }
