...
public class QrCodeUtils {
    ...
    public static Result decodeFromPhoto(Bitmap bitmap) {
        ...
        if (bitmap != null) {
            Bitmap smallBitmap = zoomBitmap(bitmap, bitmap.getWidth() / 2, bitmap.getHeight() / 2);// 为防止原始图片过大导致内存溢出，这里先缩小原图显示，然后释放原始Bitmap占用的内存
    ...
    public static Flowable<Bitmap> createQrCodeAsnyc(final String text,final int w,final int h,final Bitmap logo){
        return Flowable.fromCallable(new Callable<Bitmap>() {
            @Override
            public Bitmap call() throws Exception {
                return CodeUtils.createImage(text, w, h, logo);
            }
